...
def _pre_process_image(img_path: str, greyScale: bool = False) -> np.array:
    ...
    try:
        ...
        if greyScale:
            ...
            # ResNet50 expects 3 channels, so we duplicate the grayscale channel
...
def _reassign_orphans_dynamic(wsi_df: pd.DataFrame) -> pd.DataFrame:
    """
    Dynamically identifies orphan clusters and merges them.
    FIX: Adjusted threshold to 'typical_size / 2' to prevent breaking valid
    partial clusters (e.g., size 2 when typical is 3).
    """
    if wsi_df.empty:
        return wsi_df
    ...
    # 1. Determine Typical Size (Mode)
    typical_size = counts.mode().max()

    # 2. Identify Orphans (UPDATED LOGIC)
    # Use 50% of typical size as the cutoff.
    # For typical=3: threshold=1.5. Size 1 is orphan; Size 2 is kept.
    orphan_threshold = typical_size / 2.0
    orphan_cluster_ids = counts[counts < orphan_threshold].index.tolist()

    if not orphan_cluster_ids:
        return wsi_df

    # 3. Calculate Centroids for ALL clusters
    centroids = {}
    unique_clusters = wsi_df["cluster_id"].unique()

    for cid in unique_clusters:
        # Extract vectors for this cluster
        cluster_vectors = np.vstack(wsi_df[wsi_df["cluster_id"] == cid]["vector"].values)
        centroids[cid] = np.mean(cluster_vectors, axis=0)

    # 4. Reassign Orphans
    for idx, row in wsi_df.iterrows():
        current_cid = row["cluster_id"]

        if current_cid in orphan_cluster_ids:
            current_vector = row["vector"]
            best_cid = current_cid
            min_dist = float("inf")

            # Find nearest neighbor among all other clusters
            ...
            # Update assignment
            wsi_df.at[idx, "cluster_id"] = best_cid
...
def _run_clustering(df: pd.DataFrame, threshold: float) -> pd.DataFrame:
    ...
    for wsi_id, group in grouped:
        ...
        final_wsi_df = _reassign_orphans_dynamic(wsi_df)
...
async def cluster_from_directory(request: DirectoryRequest = Body(...)):
    ...
    try:
        ...
        # 3. Clustering
        threshold = request.threshold or 0.06
        clustered_df = _run_clustering(df_data, threshold)
...
async def tune_parameters(request: TuningRequest = Body(...)):
    ...
    try:
        ...
        test_thresholds = np.arange(0.01, 1.0, request.early_stop_stepsize).tolist()
